...
class MetricModelSquadService():
    ...
    def __getPointsMetricBySquad(self,squads:pd.DataFrame,baseActivos:pd.DataFrame,
        ...
        squads["applyPractice"] = squads.apply(lambda record: self.__hazAzure(record["app"],metricsDataAzure),axis=1)
    ...
    def calculateMetricAssesmentAzureSqlBySquad(self,metricsAssesment:pd.DataFrame,baseActivos:pd.DataFrame)->pd.DataFrame:
        squads:pd.DataFrame = self.__squadPrioritizedRepository.getSquadsByServiceCloud(Constants.PATH_INPUT_SQUADS_PRIORIZADOS_HOJA_AZURE_SQL)

        squads = self.__getPointsAssesmentMetricBySquad(squads,baseActivos,
                                                Constants.ASSESMENT_METRICS_AZURE_SQL,metricsAssesment
                                                )
        
        self.__maturityLevelService.calculateMaturityLevelAssesmentBySquad(Constants.SERVICE_CLOUD_AZURE_SQL,squads)

        return squads
    
    def calculateMetricAssesmentRedisCacheBySquad(self,metricsAssesment:pd.DataFrame,baseActivos:pd.DataFrame)->pd.DataFrame:
        squads:pd.DataFrame = self.__squadPrioritizedRepository.getSquadsByServiceCloud(Constants.PATH_INPUT_SQUADS_PRIORIZADOS_HOJA_AZURE_REDIS)
        ...
        self.__maturityLevelService.calculateMaturityLevelAssesmentBySquad(Constants.SERVICE_CLOUD_CACHE_REDIS,squads)

        return squads    
    
    def calculateMetricAssesmentCosmosDbBySquad(self,metricsAssesment:pd.DataFrame,baseActivos:pd.DataFrame)->pd.DataFrame:
        squads:pd.DataFrame = self.__squadPrioritizedRepository.getSquadsByServiceCloud(Constants.PATH_INPUT_SQUADS_PRIORIZADOS_HOJA_AZURE_COSMOS)
        ...
        self.__maturityLevelService.calculateMaturityLevelAssesmentBySquad(Constants.SERVICE_CLOUD_COSMOS_DB,squads)

        return squads        
    
    def __getPointsAssesmentMetricBySquad(self,squads:pd.DataFrame,baseActivos:pd.DataFrame,
                                 metricsListAssesment:list[str],metricsDataAssesment:pd.DataFrame)->pd.DataFrame:        
        squads["app"] = squads.apply(lambda record: self.__getAppsBySquad(record["squadCode"],baseActivos),axis=1)
